...
for i = 2:nsamples
    ...
    %
    [dis, dis_i] = sort(distances);
    num = min(length(distances), k);
    closest_nodes = dis(1:num);
    closest_nodes_i = dis_i(1:num);
    
    for j = 1:num
        closest_node = dis(j);
        closest_node_i = dis_i(j);
        if(LocalPlanner(x, samples(:,closest_node_i)))
            nedges = nedges + 1;
            edges(nedges, :)=[i, closest_node_i];
            edge_lengths(nedges) = closest_node;
        end
    end
